...
from src.inpainting.inpaint import train
import numpy as np
import os
def inpaint_dataset(mode):
    if not os.path.exists('data/inpaint_runs'):
        os.mkdir('data/inpaint_runs')
...
def load_sample(s, mode):
    ...
    if not bar_box:
        ...
        if crop:
            ...
            x1, y1 = crop[1]
            img = img[y0:y1, x0:x1]
        plt.imsave('data/micrographs_final', img)
        return
